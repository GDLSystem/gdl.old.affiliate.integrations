...
namespace Gdl.Affiliate.Integrations.AffiliateConversions
{
    ...
    public class AffiliateConversionDomainService : BaseDomainService, IAffiliateConversionDomainService
    {
        ...
        public AffiliateConversionDomainService(IShopinessDomainService shopinessDomainService, IShopinessConversionRepository shopinessConversionRepository, IRepository<UserAffiliate, Guid> userAffiliateRepository)
        {
        ...
        {
            var outdatedAffs = await _userAffiliateRepository.GetListAsync
            (
        ...
        public async Task SyncAffConversions(DateTime fromDate, DateTime toDate, AffiliateOwnershipType affiliateOwner)
        {
            ...
            var existingUserAffs = await _userAffiliateRepository.GetListAsync(x=> x.CreatedAt <= DateTime.UtcNow.AddMonths(-3));
            var existingConversions = await _shopinessConversionRepository.GetListAsync(_ => _.ConversionTime >= fromDate.ConvertToUnixTimestamp());
            ...
            {
                var newConversions = new List<ShopinessConversion>();
                ...
                if (newConversions.IsNotNullOrEmpty())
                {
                    ...
                    {
                        await _shopinessConversionRepository.InsertManyAsync(batch);
                    }
